...
@section('title', 'Keranjang Sampah Dokumen Masuk')
@section('content')
            ...
            <div class="inner-body">
                ...
                <div class="page-header" style="margin-bottom: 20px;">
                    <div class="flex-row d-flex justify-content-between align-items-center w-100">
                        ...
                        <a href="{{ route('admin.arsip_masuk') }}" class="btn btn-dark">
                    ...
                    <div class="col-md-12" id="left-panel">
                        <div class="card custom-card">
                            <div class="pb-0 card-header border-bottom-0">
                                <div>
                                    <div class="d-flex">
                                        <label class="pt-2 my-auto main-content-label">SAMPAH DOKUMEN MASUK</label>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table mb-0 table-bordered" id="dokumenMasuk-tabel" style="width: 100%">
                                        <thead>
                                        ...
                                        <tbody>
                                            @foreach ($arsip_masuk as $item)
                                                <tr style="text-align: center;">
                                                    ...
                                                    <td class="text-wrap"
                                                        onclick="showDetails('{{ $item->dokumen_kategori ? $item->dokumen_kategori->nama_kategori : 'Tidak ada kategori' }}','{{ $item->nama_dokumen }}', '{{ $item->pengirim }}', '{{ $item->penerima }}', '{{ $item->instansi->nama_instansi }}', '{{ $item->tanggal_masuk }}', '{{ $item->lampiran }}')">
    ...
    <script>
        function showDelete(id, nama_dokumen) {
            Swal.fire({
                ...
                text: "Menghapus Data" + " " + nama_dokumen + " " + "secara permanen",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{ url('/admin/arsip_masuk/sampah/delete/') }}/" + id;
                    Swal.fire({
                        title: "Hapus Data!",
        ...
        function showRestore(id, nama_dokumen) {
            Swal.fire({
                title: "Apakah Anda yakin?",
                text: "Mengembalikan Data" + " " + nama_dokumen + " " + "ke dalam kategori",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{ url('/admin/arsip_masuk/sampah/restore/') }}/" + id;
                    Swal.fire({
